...
async function createApp(module: ModuleType, apiPrefix?: string) {
  const app = await NestFactory.create(module, OakRouter);
...
Deno.test("fetch success", async (t) => {
  ...
  await t.step("fetch ok with global prefix, change with body", async (it) => {
    @Controller("/")
    class A {
      @Get("/")
      b(context: Context) {
...
Deno.test("use middleware", async (t) => {
  ...
  await t.step("middleware with app.get error", async () => {
    const app = await NestFactory.create(AppModule, OakRouter);
    const callStack: number[] = [];
    app.get("/", () => {
      throw new Error("error");
